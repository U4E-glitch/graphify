...
- Fix/Feat: the MCP server starts on a fresh install again, and now supports mcp 2.x (#1729). The `mcp` extra was unpinned, so `pip install "graphifyy[mcp]"` resolved the new mcp 2.x — which removed the low-level `Server` decorator API (`@server.list_tools`/`call_tool`/`list_resources`/`read_resource`) that `serve.py` is built on. Every server start, stdio and `--transport http` alike, died at import behind a misleading "mcp not installed. Run: pip install ..." message that sent users back to reinstall the package they already had. Registration now goes through `_register_handlers`, which uses the 1.x decorators when present and 2.x's `add_request_handler(method, params_type, handler)` otherwise; the handler bodies are shared, since 2.x kept the camelCase field aliases with `populate_by_name`. Two further breaks came with it: the tool schema field is readable as `inputSchema` on 1.x but only `input_schema` on 2.x (the alias populates on construction but is not an attribute), which broke the `project_path` injection and so all of `tools/list`; and `Resource.uri`/`TextResourceContents.uri` went from `AnyUrl` to `str`, so passing an `AnyUrl` failed validation and broke `resources/list`. The schema field is now resolved at runtime and URIs are passed as plain strings, which validate on both majors. The extra (and `[all]`) allow `mcp<3` — the ceiling is deliberate, since 2.0 dropped the decorators with no deprecation window. Verified against mcp 1.30.0 and 2.2.0: identical `tools/list`, `tools/call`, `resources/list` and `resources/read` payloads over both stdio and HTTP, with the full suite showing the same pre-existing failures on each.
- Build/CI: `uv.lock` moves to mcp 2.2.0 and records the `<3` specifier, and a `mcp-compat` job runs the server suite against each mcp major (#1729). The lock still pinned mcp 1.27.1 with no specifier, and CI installs with `uv sync --frozen` — which, unlike `--locked`, skips the freshness check — so CI resolved 1.27.1 and its green checkmark said nothing about the 2.x support it was meant to validate. The lock now matches `pyproject.toml`, so the `test` matrix exercises 2.x; because a lock can only hold one major at a time, `mcp-compat` installs `mcp<2` and `mcp>=2,<3` into throwaway venvs (not `--frozen`, so the committed lock is never touched) and runs `test_serve.py`/`test_serve_http.py` against both.
- Test: `resources/list`, `resources/read` and the unknown-resource error path are now exercised over the wire, and every tool's schema is asserted to carry `project_path`. The resource handlers had no wire coverage at all, which is why the mcp 2.x `AnyUrl` break reached a passing suite.
